...
3. **Python**: **3.10 or newer** on your machine. If you do not have it yet, download an installer from the [Python website](https://www.python.org/downloads/) and follow the prompts (check the option to add Python to `PATH` on Windows if the installer offers it).
...
MIT (same as the [lionscraper](https://pypi.org/project/lionscraper/) package on PyPI).
